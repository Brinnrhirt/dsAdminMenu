...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('dsAdminMenu:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('dsAdminMenu:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        ESX.ShowNotification(_U("success.blips_activated"), "success")
    else
        ShowBlips = false
        ESX.ShowNotification(_U("error.blips_deactivated"), "error")
    end
end)

RegisterNetEvent('dsAdminMenu:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        ESX.ShowNotification(_U("success.names_activated"), "success")
    else
        ShowNames = false
        ESX.ShowNotification(_U("error.names_deactivated"), "error")
    end
end)

RegisterNetEvent('dsAdminMenu:client:Show', function(players)
